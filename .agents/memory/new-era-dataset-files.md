...
- **Placing a brand-new state's file (no existing era-chain for it yet):** pick the region folder by geographic footprint, not political lineage — put it alongside whichever folder already holds earlier/later states occupying the same territory (e.g. a Black Sea successor state's file belongs in the region folder that already has that area's BC-era kingdoms), even if the new state isn't a direct continuation of those sibling files' polities.
